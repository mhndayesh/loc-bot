...
from src.paths import (
    PROJECT_ROOT, FRONTEND_DIR, CONFIG_FILE, STATE_FILE, JOURNAL_FILE,
    SCRATCHPAD_FILE, CORE_DIR, MEMORY_DIR
)

BASE_DIR = PROJECT_ROOT
GUI_DIR = FRONTEND_DIR
...
class AgentAPIHandler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/api/status":
            ...
            # Count pulse files (now ChromaDB entries)
            pulse_count = 0
            try:
                stats = memory.get_stats()
                pulse_count = stats.get("entries", 0)
            except Exception:
                pass
