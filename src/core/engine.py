...
from src.paths import (
    PROJECT_ROOT, SKILLS_DIR, MEMORY_DIR, WORKSPACE_DIR, STATE_FILE,
    JOURNAL_FILE, SUMMARY_FILE, SCRATCHPAD_FILE, CONFIG_FILE, DOCS_DIR, LOGS_DIR
)
...
class AgentEngine:
    ...
    def run_tool(self, name: str, args: list) -> str:
        ...
        if name == "memorize" and len(args) >= 1:
            try:
                content = args[0]
                mem_id = memory.memorize(content)
    ...
    def agentic_recall(self, user_query, max_budget=6000):
        ...
        if target_group_id:
            try:
                ...
                group_results = memory.get_by_group(target_group_id)
